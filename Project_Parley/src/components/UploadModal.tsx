...
import type { Intent, VideoInsert } from "../lib/types";
...
export default function UploadModal({ open, onClose, onPublished }: Props) {
  ...
  const [intents, setIntents] = useState<Intent[]>(["invest_now"]);
  ...
  async function publish() {
    ...
    try {
      ...
      const payload: VideoInsert = { user_id: user.id, url: pub.publicUrl, title: title.trim() || "Untitled pitch", description: description.trim(), stage: stage.trim() || null, sector: sector.trim() || null, tags: [], intent: intents.length > 0 ? intents : ["open_to_talk"] };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-md" onClick={onClose}>
      <div className="card w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Body: single column to prevent any overlap */}
        <div className="grid gap-4 p-5">
          {/* Record / Upload buttons */}
          <div className="flex gap-2">
            {!mediaRef.current?.state || mediaRef.current.state === "inactive" ? (
              <button className="btn-accent flex-1" onClick={startRecording} disabled={busy}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="6" /></svg>Record
              </button>
            ) : (
              <button className="btn-dark flex-1" onClick={stopRecording}>
          ...
          {/* CTA intent selector */}
          <div>
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-[0.15em] text-muted">Call to action (select any)</label>
            <div className="flex flex-wrap gap-1.5">
              {(Object.keys(INTENT_LABELS) as Intent[]).map((key) => {
                const active = intents.includes(key);
                return (
                  <button key={key} type="button" onClick={() => setIntents((prev) => prev.includes(key) ? prev.filter((x) => x !== key) : [...prev, key])} className={`rounded-full px-3 py-1.5 text-[11px] font-medium transition ${active ? "bg-gradient-to-br from-accent-from to-accent-to text-white" : "bg-surface-2 text-muted-soft ring-1 ring-line hover:text-white"}`}>{INTENT_LABELS[key]}</button>
                );
              })}
            </div>
          </div>

...
type AuthState = {
  ...
  signUp: (email: string, password: string, userType: UserType, name: string, username?: string) => Promise<void>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthState>(() => ({
    ...
    async signUp(email, password, userType, name, username = "") {
      ...
      if (error) throw error;
      const u = data.user;
      if (u) await supabase.from("profiles").upsert({ id: u.id, user_type: userType, name, username: cleanUsername || null });
    },
    async signOut() {

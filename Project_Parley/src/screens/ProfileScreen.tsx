  ...
  return (
    <div className="shell px-5 py-8 sm:px-8">
      <div className="card-ink p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-center gap-5">
            ...
            <div>
              <div className="eyebrow">{profile?.user_type === "investor" ? "Investor" : "Founder"}</div>
      ...
      {videos.length === 0 ? (
        ...
        <div className="mt-3 grid gap-5 sm:grid-cols-2">{videos.map((v) => <VideoCard key={v.id} video={v} profile={profile} />)}</div>
      )}
    </div>
  );
}

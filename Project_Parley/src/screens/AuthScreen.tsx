...
export default function AuthScreen({ mode }: { mode: Mode }) {
  ...
  const userTypes: UserType[] = ["founder", "investor", "casual"];
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      else await signUp(email, password, userType, name, username);
      navigate("/onboarding");
    } catch (e) {
          ...
          {mode === "signup" && (
            <>
              ...
              <div>
                ...
                <input className="input" placeholder="your handle (for inbox)" value={username} onChange={(e) => setUsername(e.target.value)} required />
              </div>
              <div>

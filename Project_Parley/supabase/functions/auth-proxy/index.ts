...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Forward to Supabase Auth using the anon key (so RLS still applies to
    // any session we return). We use the service-role admin client only for
    // the rate-limit RPC; the actual auth call goes through a fresh anon
    // client so we never elevate the user.
    ...
    if (body.kind === "signup") {
      ...
      // Best-effort profile insert. RLS allows the user to insert their own
      // row; if the session isn't confirmed yet (email confirm off), do it
      // from the service role so onboarding doesn't break.
      if (data.user) {
        ...
        if (profileErr) {
          // Non-fatal — return session anyway.
    ...
    if (error) {
      // Leave success=false so failed attempts keep counting.
      return new Response(JSON.stringify({ error: error.message }), {
        status: 401,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
